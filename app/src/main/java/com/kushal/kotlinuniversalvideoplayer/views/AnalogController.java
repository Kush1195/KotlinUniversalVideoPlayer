package com.kushal.kotlinuniversalvideoplayer.views;
...
import com.kushal.kotlinuniversalvideoplayer.activities.SplashActivity;
...
//        textPaint.setTextSize(33 * MainActivity.ratio);
...
//        circlePaint2.setColor(Color.parseColor("#FFA036"));
        ...
        linePaint = new Paint();
        linePaint.setColor(Color.parseColor("#123456"));
//        linePaint.setColor(Color.parseColor("#FFA036"));

        WindowManager wm = (WindowManager) getContext().getSystemService(Context.WINDOW_SERVICE);
        Display display = wm.getDefaultDisplay();

        linePaint.setStrokeWidth(7 * Math.min((float) display.getHeight() / (float) 1920, (float) display.getWidth() / (float) 1080));
        angle = "0.0";
        label = "Label";
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int ang = 0;
        float x = 0, y = 0;
        int radius = (int) (Math.min(midx, midy) * ((float) 15.2 / 16));
        ...
        for (int i = (int) (deg2); i < 22; i++) {
            ...
            circlePaint.setColor(Color.parseColor("#aaaaaa"));
        ...
        for (int i = 3; i <= deg3; i++) {
            ...
            x = midx + (float) (radius * Math.sin(2 * Math.PI * (1.0 - tmp)));
            y = midy + (float) (radius * Math.cos(2 * Math.PI * (1.0 - tmp)));
            circlePaint2.setColor(Color.parseColor("#000000"));
        ...
        circlePaint.setColor(Color.parseColor("#6F001A"));
        canvas.drawCircle(midx, midy, radius * ((float) 12.3 / 15), circlePaint);
